...
Mer_Sales = read.csv(file = "CART1.csv", header = T)
...
CartFullModel = rpart(formula = Annual_Sales ~ . ,
                      data = Mer_SalesTrainUncapped[,-1], method = "anova")
CartFullModel
summary(object = CartFullModel)
summary(Mer_SalesTestUncapped[,'Annual_Sales'])
names(Mer_SalesTrainUncapped)
# Plot the Regression Tree
rpart.plot(x = CartFullModel, type = 4,fallen.leaves = T, cex = 0.7,
           shadow=c('brown', 'green','red'), nn=T)
