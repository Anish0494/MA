...
fit1
summary(fit1)
#residuals-> difference between predictive value and actual values of y
#predictive's Y - > Y = 0.9645 + 1.6699*x
#we have to minimize the residuals
#R-squared gives the %age of variance of Y is explained by X
#Adjusted R-squared penalises y
#F- statistic with P valus is less than .05 then we say it is a significant model
#and atleast there is anyone of the independent variable which is related to y
names(fit1)
...
# Linearity
plot(x=X, y=residuals(fit1))#testing the linearity 
#and if there is no  pattern of increase and decrease of point i.e scatter then it is linear
plot(residuals(fit1))
car::durbinWatsonTest(fit1)#independence of error
#if value of D-W statstic is near to 2 then there is no auto-co-relation
#if more than 2 then neagtive co-relation and if less than then +ve co-relation 
...
#IF LINES PASS THROUGH ALL THE POINTS THEN IT IS STANDERED 
...
#AS THE POINT IS SCATTERED IN THE RANGE THEN IT IS EQUAL VARIANCE AND IF IT IS 
#LIKE A FUNNEL THEN IT IS NOT A EQUAL VARIA
