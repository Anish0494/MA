...
summary(logit)
#as there is no '*' in income so the model does not depend on income
exp(coef(logit))
#as the logistic regression take interprtation by log so we have to take anti-log

logit = glm(default ~  balance + student, family='binomial', data=Default)
summary(logit)
exp(coef(logit))
library(dplyr)
